...
def DisplayCustStoreTotals(custNames, custAddrs, custTires, custCosts):
    ...
    while i < lenCust:
        ...
        print(" *** The total number of tires purchased:  ", numSTires)
        print(
            " *** The total sales for the tires including installation: $", totalSCost
        )
        ...
        print(
            "{:<20}\t{:<20}\t{:<10}\t{:<10} ".format(
                "Name", "Address", "No. Tires", "Total Cost"
            )
        )
        print(
            "{:<20}\t{:<20}\t{:<10}\t{:<10} ".format(
    ...
    while i < lenCust:
        print(
            "{:<20}\t{:<20}\t{:10d}\t{:10.2f} ".format(
                custNames[i], custAddrs[i], custTires[i], custCosts[i]
            )
        )
        i = i + 1
    ...
    print(
        "{:<20}\t{:<20}\t{:10d}\t{:10.2f} ".format(
            storeID, "Store totals:", numSTires, totalSCost
        )
    )
